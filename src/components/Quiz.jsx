      ...
      <div className="md:w-9/12 w-[90%] flex md:flex-row flex-col-reverse mx-auto">
        ...
        <div className="md:w-[70%] w-full">
          <div>
            {questions.map((question, index) => (
              ...
              >
                <p className="flex items-center rounded text-xs p-2 cursor-pointer">
                  <span className="h-8 w-8 bg-blue-700 rounded-full flex justify-center items-center text-white mr-3">
                    {index + 1}
                  </span>
                ...
                <div className="">
                  {question.options.map((option, index) => (
                    <div
                      className={`flex gap-5 pb-1 border border-gray-200 rounded text-xs p-2 cursor-pointer mt-3 ${answers[question.id] === option ? "bg-gray-300" : ""
                        }`}
                      key={option}
                      onClick={() => handleAnswerSelect(question.id, option)}
                    >
                      <p className='text-[10px] mb-1 text-slate-500'>
                        {index + 1 === 1 ? 'a' : index + 1 === 2 ? 'b' : index + 1 === 3 ? 'c' : index + 1 === 4 ? 'd' : ''}
                      </p>
            ...
            <button onClick={handleSubmit} className="bg-blue-700 px-6 py-2 text-white rounded">
              Submit Quiz
            </button>
          </div>
        </div>
        ...
        <div className="md:w-[30%] w-full p-4">
          {showResult && (
            <div>
              ...
              <button
                onClick={restartQuiz}
                className="bg-blue-700 text-white w-full py-2 rounded mt-16"
              >
                Restart
              </button>
            </div>
